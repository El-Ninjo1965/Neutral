...
| Permanenter Post-Deployment-Smoke | CODE UND TEST BESTANDEN | Der FTPS-Job prüft danach rein lesend Root, SPA-Rewrite, Adminschutz, Status- und Modul-API, Viewer-GPS, internen PHP-Schutz, exakten Commit/Basispfad und die ausgelieferten Modulverträge. Das Gesamtergebnis wird ohne Logdaten als Commitstatus `production/ftps-http` veröffentlicht. Der erste produktive Lauf dieses dauerhaften Gates bleibt bis zum Statusnachweis offen. |
