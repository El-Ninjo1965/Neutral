...
The current verified state is: the dedicated web-app FTP account is a valid chrooted root `/` target containing the public bundle, including `index.html`, `style.css`, `user-app.js`, `api-client.js`, and `platform/`. The repository-side root cause was the deploy script itself: `--web-app` mode had not been honored, so the wrong allowlist was staged for upload. That script bug has been corrected in `scripts/manual-ftps-deploy.js` so the public web-app bundle is now staged for the dedicated web-app target and not the server path. The verified live runtime path is `/index/web-app/` and resolves to `/home/web1819/public_html/index/web-app/`; the live HTTPS bundle matches the exact files uploaded to the FTPS root, and the public API login/session flow for the Developer test user succeeds against `/index/app/neutral/webroot/api/auth/login` and `/api/auth/me`.
