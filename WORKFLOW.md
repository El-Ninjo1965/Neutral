...
- **Connector-Nachweis:** Ein separater Abschlussjob setzt ausschließlich `production/ftps-http` auf Erfolg oder Fehler. Bei einem Fehler nennt er nur die begrenzte Stufe `target`, `tests`, `package`, `client`, `upload` oder `smoke`. Dadurch bleibt der Push-Workflow über den GitHub-Connector überprüfbar, ohne Actions-Logs oder Geheimnisse zu lesen.
