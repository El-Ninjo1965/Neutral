...
- **GPS-Live-Befund nach aktuellem realem Test (Google Chrome auf iPadOS): BESTANDEN:** Secure Context **JA**, **Protokoll: https:**, Frame **NEIN**, Permission **granted**, `getCurrentPosition` erfolgreich aufgerufen, reale Position ermittelt, Ergebnis **success**. Die GPS-Diagnose wurde anschließend wie geplant aus der normalen User-UI entfernt. Der GPS-Live-Test ist damit als bestanden dokumentiert, separat von der offenen Offline-/Warmstart-Liveabnahme.
...
### 2026-09-05 – GPS Live-Root-Cause (Chrome auf iPadOS) und Navigation-State

- **Korrektur der Dokumentation:** Der reale Nachtest der Diagnose wurde mit **Google Chrome auf iPadOS** durchgeführt, nicht mit Safari. Die allgemeine Safari-/WebKit-Abnahme bleibt separat offen; Einträge, die diesen konkreten Test als Safari-Test bezeichneten, wurden korrigiert.
- **Aktueller realer Diagnosebefund (Chrome/iPadOS, Produktion):** Secure Context **JA**, Geolocation API JA, Permissions API JA, Permission State `granted`, `getCurrentPosition` aufgerufen JA, Ausgang `success`, reale Position ermittelt, kein Fehlercode. Damit ist der aktive GPS-Livetest auf echt implementiertem Secure Context und echter Browser-Standortermittlung als erfolgreich dokumentiert.
- **Root Cause `PERMISSION_DENIED`:** Der frühere HTTP-/Secure-Context-Fehler wurde im aktuellen Live-Test nicht mehr reproduziert. Der aktuelle Status bleibt technisch korrekt: Geolocation erfordert im unsicheren Kontext einen Browserblock; die aktuelle HTTPS-/Secure-Context-Umgebung erfüllt die Voraussetzung und erlaubt die lokale Positionsabfrage.
- **Root Cause `Secure Context: NEIN` (Soweit aus dem Repository bestimmbar):** Der produktive Host liefert vor der eigentlichen App eine JavaScript-Challenge-/Reload-Seite („One moment, please…“, offener `openresty`-Edge, keine Weiterleitung auf HTTPS erzwungen – `http://turbolikes.com/` antwortet mit 200 statt Redirect). Die App selbst enthält keine iframes und keine `http://`-Ressourcen; TLS-Zertifikat (Let's Encrypt) ist gültig. Der nicht sichere Window-Kontext entsteht **außerhalb des Repositorys** im Hosting-/Edge-Pfad (Bot-/Challenge-Schicht bzw. Aufruf über `http://` ohne HTTPS-Zwang). **Konkrete Betreiberprüfung:** (1) Seite ausschließlich über `https://` aufrufen und prüfen, ob die Adresszeile wirklich `https://` zeigt, (2) im Hosting eine erzwungene HTTP→HTTPS-Weiterleitung aktivieren, (3) prüfen, ob eine Anti-Bot-/Challenge-Funktion des Hosters oder CDN die eigentliche App-Seite ersetzt oder einbettet, (4) Diagnosepanel liest nun zusätzlich `Protokoll` und `Eingebettet (Frame)` direkt auf dem Gerät aus. Keine spekulative Hosting-Änderung wurde vorgenommen.
- **Vertragsfehler Neutral-Consent vs. Browser-Permission (test-first korrigiert):** `confirmLocationRequest(false)` setzte bisher `permissionState = 'denied'` und tarnte damit eine Neutral-Sessionentscheidung als Browser-Verweigerung. Korrigiert: Ein Neutral-`Nein` verändert den Browser-Permission-State nicht mehr; `denied` entsteht ausschließlich aus echter Browserantwort (Permissions-API oder Fehlercode 1). Das erklärt auch den Livewert „letzte Neutral-Entscheidung: JA“ (Auto-Pfad ruft `confirmLocationRequest(true)` ohne sichtbares Modal) – gespeicherte Entscheidung und Browser-Permission sind jetzt sauber getrennt.
- **UX bei echtem Browser-Denied:** Normale UI zeigt jetzt kurz `Standortzugriff nicht erlaubt. Bitte Standortzugriff für diese Seite in den Browser- bzw. Geräteeinstellungen aktivieren.` – ehrliche Hilfe statt Fake-Aktion; keine langen technischen Meldungen.
- **Navigation-State, Root Cause 2 (realer Bug bestand weiter):** Neben der Label-Vermischung enthielt die statische Shell [index.html](/workspaces/Neutral/Web-App/public/index.html) einen hart codierten Placeholder `<span class="user-app-nav-item active">Neutral</span>`. Die ArbeitsHypothese zum Kartenpfad wurde geprüft: Der Kartenpfad ruft `renderModule()` und setzt `state.activeView = 'module:gps'` korrekt; der Fehler lag am statischen Markup, das unabhängig vom View-State einen „aktiven“ Eintrag vortäuschte. Der Placeholder wurde entfernt; Active-State wird ausschließlich generisch aus `state.activeView` abgeleitet (für jedes zukünftige Modul, kein GPS-Sonderfall).
- **Diagnosepanel:** bleibt **TEMPORÄR** erhalten (jetzt zusätzlich mit `Protokoll` und `Eingebettet (Frame)`), bis der Betreiber den Secure-Context-Befund auf Chrome/iPadOS erneut geprüft hat.
- **Bewusst nicht bearbeitet:** Startseiten-Umbau (Modulkarten auf Startseite / Admin-wählbares Startmodul) und persistenter Login bleiben offene, separat zu entscheidende Core-Punkte.
- **Test-first / Verifikation:** 4 neue Verträge schlugen zunächst rot fehl (Neutral-`Nein` ≠ Browser-`denied`; Protokoll/Framing in Diagnose; statischer Shell-Placeholder ohne Fake-Active-State; ehrliche Denied-UX). Danach: fokussierte Suite **71/71 bestanden**; vollständige Suite **349/349 bestanden**, 0 Fehler; `node --check` und `git diff --check` bestanden. PHP nicht betroffen.
- **Abschlussstatus Root-Cause-Block:** Commit `9a373d910f168f64e24214f5b72a443cb5eea243` wurde nach `main` gepusht; der reguläre FTPS-Deploymentlauf `33949636654` ist erfolgreich (Workflow-Tests, Paketbau, FTPS-Upload, read-only Produktions-Smoke). `HEAD == origin/main`, Arbeitsbaum sauber, `git diff --check` bestanden. Der nächste reale Chrome-/iPadOS-Test muss nach der betreiberseitigen Secure-Context-Prüfung (HTTPS-Zwang, Challenge-Schicht) erfolgen; die Safari-Abnahme bleibt separat offen.

### 2026-09-05 – Offline-First-/Warmstart-Performance vor Core-Freeze

- **Realer Live-Befund (Chrome auf iPadOS):** Die App startet sichtbar in zwei Ladephasen: Phase A `STARTING – Preparing offline-capable workspace…` (ca. 2–3 s), danach Phase B `Loading available modules…` (weitere ca. 4–5 s), erst dann erscheinen Module/GPS. Für den Offline-First-Core ist das im Warmstart nicht akzeptabel.
- **Gemessener Startpfad (Codeanalyse + Produktionsproben):** Phase A = Dokument + 36 seriell deklarierte Core-Scripts (jeweils eigener HTTP-Request auf dem Edge-Host, gemessen ~0,8–1,2 s pro API-/Asset-Request) plus Core-Minimalstart; die statische Shell selbst blockiert keinen JS-Start (Scripts sind `defer`, Shell-Text sofort im HTML). Phase B = `CoreStartup.startBackground()` führt Storage → Fassaden-Init → `ModuleManager.discoverModules()` seriell aus; Discovery wartete bisher vollständig auf den Remote-Katalog (`fetch /api/modules`, `no-store`), und erst danach wurden Modul-Entries per separatem `fetch` geladen. Ein gültiger anonymer Katalogcache existierte zwar (`neutral.module-catalog.anonymous.v1:<base>`), wurde aber **nur bei Remote-Fehler** gelesen — nicht priorisiert beim Warmstart. Das ist die Implementierungslücke hinter den 4–5 s.
- **Korrektur (Local-first Hydration, innerhalb des bestehenden Vertrags):** `readModuleCatalog()` hydriert beim Warmstart sofort aus dem gültigen anonymen Katalogcache und startet die Remote-Rekonziliation im Hintergrund (`module-catalog:refreshed`-Event bei neuen Daten). Erstaufruf ohne lokalen Zustand wartet weiterhin sauber auf den Remote-Katalog; Offline-Verhalten (gültiger Cache nutzbar, authentifizierte Antworten nie als anonymer Fallback persistiert, fehlerhafte Einträge fail-closed) bleibt unverändert. Modul-Entry-Assets nutzen weiterhin `no-store`, kommen aber im Warmstart aus dem HTTP-Cache des Browsers, sobald sie einmal geladen waren.
- **Doppelte Ladephase:** Phase A ist die statische Shell (First Paint nicht blockiert), Phase B war die Discovery-Wartephase. Nach der Korrektur zeigt der Warmstart sofort lokal bekannte aktive Module; der `pending`-Text erscheint nur noch beim echten First Run ohne lokalen Katalog. Die komplette Entfernung des separaten STARTING-Screens wäre ein Startseiten-/Shell-Redesign und wurde bewusst nicht begonnen.
- **VORSCHLAG – noch nicht beschlossen/umgesetzt (Performance-Budget, Warmstart modernes Mobilgerät):** Shell sichtbar < 300 ms nach Dokument; lokale Navigation/Module sichtbar < 500–800 ms; keine sichtbare mehrsekündige Discovery-Wartephase; Remote-Rekonziliation vollständig im Hintergrund. Belastbare Messwerte erfordern den nächsten realen Chrome-/iPadOS-Warmstart mit Core-Performance-Marken (`shell-visible`, `minimal-core-ready`, `module-discovery-complete`).
- **VORSCHLAG – noch nicht beschlossen/umgesetzt (Architektur):** Ein expliziter „Persisted Last Known Good State“-Vertrag (local-first hydration, background reconciliation, atomare Übernahme nur bei tatsächlicher Änderung, definierte Error-/Fallback-Zustände, Cachealter-/Deploymentinvalidierung) sollte als genereller Core-Vertrag festgelegt werden; die jetzige Änderung schließt nur die konkrete Katalog-Implementierungslücke.
- **Test-first / Verifikation:** Neuer roter Test „Warmstart mit gültigem Katalog wartet nicht auf 5-s-Remote“ schlug zunächst mit `still-waiting` fehl; danach grün. Fokussierte Suites **78/78 bestanden**; vollständige Suite **351/351 bestanden**, 0 Fehler; `git diff --check` bestanden. PHP nicht betroffen. Keine reale Warmstart-Abnahme als bestanden markiert.
- **Bewusst nicht bearbeitet:** GPS Secure-Context (Betreiber-Hostingprüfung offen), GPS-Diagnosepanel (bleibt temporär), Startseiten-Redesign, persistenter Login, Provider-/Freeze-Arbeit.
- **Abschlussstatus Warmstart-Block:** Commit `45dd4161bef3f8305667aab4fb3a6095b2632d75` wurde nach `main` gepusht; der reguläre FTPS-Deploymentlauf `33950650720` ist erfolgreich (Workflow-Tests, Paketbau, FTPS-Upload, read-only Produktions-Smoke). `HEAD == origin/main`, Arbeitsbaum sauber, `git diff --check` bestanden. Der reale Chrome-/iPadOS-Warmstart mit Performance-Marken muss anschließend durch den Betreiber gemessen werden.

### 2026-09-05 – HTTP→HTTPS-Root-Cause und Offline-First-Warmstart-Live-Regression

- **Teil A – GPS/Transport (realer Beweis, Chrome auf iPadOS):** Der aktuelle Live-Test zeigt eindeutig: Secure Context JA, **Protokoll: https:**, Eingebettet (Frame) NEIN, `getCurrentPosition` aufgerufen JA, Ergebnis success, reale Position ermittelt. Damit ist der aktive GPS-Live-Test als bestanden dokumentiert. Die separate Offline-/Warmstart-Liveabnahme bleibt offen und wird nicht als bestanden markiert.
- **Korrektur A (Repo-seitig, test-first):** Root-[.htaccess](/workspaces/Neutral/.htaccess) erzwingt nun `301` von HTTP auf `https://%{HTTP_HOST}%{REQUEST_URI}` (Pfad und Query bleiben erhalten, keine Schleife bei HTTPS, keine Canonical-Host-Umleitung). Zusätzlich setzt sie für statische JS/CSS-Assets `Cache-Control: public, max-age=86400` (HTML/API/Manifest bleiben uncached, damit neue Revisionen greifen). **Restrisiko LIVE/BETREIBERABHÄNGIG:** Wenn der vorgeschaltete OpenResty-/Edge-Layer des Hosters HTTP-Anfragen beantwortet, bevor Apache/LiteSpeed die `.htaccess` auswertet, ist zusätzlich die hosterseitige HTTPS-Erzwingung (z. B. im Control Panel / CDN) zu aktivieren. Der Produktions-Smoke prüft die HTTPS-Anforderung jetzt explizit und schlägt fehl, solange HTTP ohne Weiterleitung antwortet.
- **Smoke-Vertrag geändert:** `production-readonly-smoke.js` unterscheidet jetzt CODE-DEPLOYMENT und HTTPS-/LIVE-ANFORDERUNG: Es prüft per `redirect: 'manual'` die HTTP-Variante der öffentlichen Basis und fordert eine 3xx-Weiterleitung auf `https://`; Summary enthält `httpsEnforced: true`. Ein Deployment ohne HTTPS-Zwang wird nicht mehr als vollständig erfolgreich gemeldet.
- **Teil B – Warmstart-Regression (real NICHT BESTANDEN):** Trotz Local-first-Katalogkorrektur zeigt der echte Warmstart weiterhin zwei sichtbare Ladephasen (`STARTING` ca. 2–3 s, danach `Loading available modules…` ca. 4–5 s). Root Causes nach Codeanalyse: (1) Phase A wird durch 36 seriell geladene Core-Scripts ohne langfristige Cache-Header über den langsamen Edge-Pfad dominiert — die Shell selbst ist statisch sofort im HTML, aber der Core wird erst nach allen Script-Requests arbeitsfähig; die neuen `Cache-Control`-Header für JS/CSS adressieren genau diesen Teil sofern der Host `mod_headers` honoriert. (2) Phase B erscheint, weil die User-Shell ihren ersten Render ausführt, bevor die Core-Scripts überhaupt geladen sind — zu diesem Zeitpunkt kann noch keine localStorage-Hydration erfolgt sein, da diese im Core (`core-loader.js`) läuft. Die eigentliche Netzwerk-Wartezeit auf den Remote-Katalog ist bereits beseitigt; verbleibend ist die strukturelle Serienabhängigkeit „Scripts → Core → Hydration → Re-Render“.
- **Temporäre Startup-Diagnose:** Die Startseite enthält nun einen **TEMPORÄREN** lokalen Bereich `Startup-Diagnose (temporär)`, der alle Core-Performance-Marken (navigation-start, shell-visible, minimal-core-ready, storage-ready, module-discovery-complete, ui-interactive u. a.) in Millisekunden seit Navigationsstart anzeigt. Keine personenbezogenen Daten, keine Servertelemetrie. **TEMPORÄR – nach erfolgreicher realer Warmstart-Messung wieder entfernen.**
- **VORSCHLAG – noch nicht beschlossen/umgesetzt:** (a) Ein Service Worker mit App-Shell-/Asset-Caching würde Warmstarts (inkl. echtem Offline-Neustart im Flugmodus) erst vollständig erfüllen; das ist eine neue Core-Architekturentscheidung und wurde nicht ad-hoc eingeführt. (b) Kritische Core-Dateien könnten zu einem versionierten Bundle mit immutable-Cache zusammengefasst werden; zuerst sollen die realen Messwerte aus der temporären Startup-Diagnose die tatsächlichen Zeitfresser belegen.
- **Test-first / Verifikation:** 3 neue Verträge schlugen zunächst rot (Startup-Panel, Smoke-HTTPS-Prüfung, htaccess-301-Regel). Danach: fokussierte Suites **81/81 bestanden**; vollständige Suite **354/354 bestanden**, 0 Fehler; `node --check`, `git diff --check` bestanden; PHP nicht betroffen. Keine Warmstart-/GPS-Abnahme als bestanden markiert.
- **Bewusst nicht bearbeitet:** GPS-Logik (Transport zuerst), Startseiten-Redesign, persistenter Login, Provider-/Freeze-Arbeit.
- **Nachträgliche Smoke-Korrektur (Evidenz aus Lauf `33951678794`):** Der erste Lauf mit neuer HTTPS-Prüfung meldete fälschlich `httpsEnforced: true`, weil die Challenge-Schicht des Hosters **vor** der `.htaccess` sitzt: Die Probe auf `http://turbolikes.com/` wurde im Workflow-Umfeld beantwortet, ohne dass die Redirect-Prüfung korrekt griff; die direkte Nachprüfung zeigte weiterhin `GET http://turbolikes.com/ -> 200` (openresty, JS-Challenge „One moment, please…“). Der Smoke wurde daher korrigiert: Die HTTPS-Anforderung wird jetzt **immer** separat ausgewiesen — bei HTTP 200 ohne Weiterleitung bleibt das Code-Deployment erfolgreich, aber der Smoke meldet `httpsEnforced: false` mit klarem Hinweis statt „alles erfolgreich“. Damit ist die HTTPS-Umleitung final als **LIVE/BETREIBERABHÄNGIG** klassifiziert: Die Repo-`.htaccess` enthält die 301-Regel, doch der vorgeschaltete OpenResty-/Challenge-Layer des Hosters beantwortet HTTP, bevor Apache/LiteSpeed die Regel erreichen kann. **Erforderliche Hosting-Einstellung:** HTTPS-Erzwingung/„Force HTTPS“ im Control Panel bzw. am CDN/Edge aktivieren (bzw. die Anti-Bot-Challenge so konfigurieren, dass sie nur HTTPS bedient).
- **Finale HTTPS-Evidenz:** Die Probe im Produktions-Workflow meldet für `http://turbolikes.com/` Status **301 mit Location** (`{"httpsProbe":{"status":301,"location":"present"}}`) — vom GitHub-Runner aus greift die neue `.htaccess`-Weiterleitung also bereits. Von anderen Netzen (Codespace) beantwortet dagegen die **Imunify360-Bot-Protection** des Hosters HTTP-Anfragen mit einer eigenen 200er-Challenge-Seite („Access denied by Imunify360 bot-protection“ / „One moment, please…“), ohne die App-`.htaccess` zu erreichen. **Fazit:** Die HTTPS-Erzwingung ist repo-seitig deployed und nachgewiesen aktiv; die verbleibende HTTP-Auslieferung auf dem iPad-Netz hängt an der hosterseitigen Bot-Schutz-/Edge-Schicht. **LIVE/BETREIBERABHÄNGIG:** Im Hosting „Force HTTPS“ aktivieren bzw. Imunify360 so konfigurieren, dass Bot-Challenge nur über HTTPS ausgeliefert wird (oder die öffentliche App-URL davon ausnehmen), da sonst reale Geräte weiterhin `http:` sehen können. Der Smoke weist Code-Deployment und HTTPS-Live-Anforderung jetzt getrennt aus.

### 2026-09-05 – Service Worker / echtes Offline-First vor Core-Freeze

- **Ausgangslage (real auf Chrome/iPadOS bestätigt):** Warmstart zeigt trotz Local-first-Katalog und Cache-Headern sichtbare Ladephasen; es existierte kein Service Worker, ein kompletter Flugmodus-Neustart war nicht garantiert. Der Offline-First-Vertrag war code-seitig unvollständig.
- **Architekturentscheidung (begründet):** Reiner HTTP-Browsercache kann einen Offline-Neustart nicht garantieren (heuristische Freshness, keine programmgesteuerte Shell-Auslieferung); IndexedDB/localStorage decken Daten, aber keine Asset-Auslieferung ab. Ein kleiner Vanilla-Service-Worker mit Cache Storage API ist die minimal ausreichende, frameworkfreie Lösung — keine neue Build-Pipeline, keine PWA-Bibliothek.
- **Umsetzung:** `Web-App/public/service-worker.js` mit versioniertem Cache `neutral-shell-v<Deploy-Stamp>` (Test-Hook `__NEUTRAL_SW_VERSION_OVERRIDE__`; spätere produktive Stamp-Quelle ist das Build-/Deployment-Manifest, kein hart codierter Versionswert). Precache beim `install`: Shell-HTML, `style.css`, alle 32 Core-Skripte und die öffentlichen User-Skripte. Navigation: network-first mit Fallback auf die gecachte Shell. Statische Core-/Modulassets: cache-first mit Hintergrund-Refresh. Modul-Entry-Skripte (`Web-App/app/modules/<id>/index.js`) bleiben nach einmaligem Laden offline nutzbar. `activate` entfernt alle älteren `neutral-shell-v*`-Caches (kein Mischzustand). Registrierung nur unter `window.isSecureContext`, nicht-blockierend, Fehler beim Registrieren brechen den Onlinebetrieb nicht.
- **Sicherheitsgrenzen:** Nur GET; `/api/`, Auth-/Session-, Admin- und Setup-Endpunkte werden nie aus dem Cache beantwortet; keine Loginantworten, Sessions, personenbezogenen API-Antworten oder Secrets im Cache.
- **Versionierung/Update:** Der Service Worker selbst wird per `.htaccess` mit `no-cache` ausgeliefert (Update-Fähigkeit), über die Root-Rewrite-Regel am Scope-Root bereitgestellt; neue Deployment-Stamps erzeugen neue Caches, alte werden beim Aktivieren entfernt.
- **Drei Zustände:** A) First Run online lädt vollständig und baut den Shell-Cache auf; B) Warmstart online liefert Shell/Core sofort aus dem Cache mit Hintergrund-Rekonziliation; C) Offline-Start nach erfolgreichem First Run liefert Shell, Core und lokal bekannte Modul-Entries vollständig lokal — netzwerkabhängige Funktionen melden sich lokal, ohne die App zu blockieren.
- **HTTPS-Abhängigkeit:** Service Worker benötigt Secure Context. CODE-SEITIG ERLEDIGT; LIVE/BETREIBERABHÄNGIG bleibt, dass Chrome/iPadOS tatsächlich über `https:` läuft (Imunify360-/Hosting-HTTPS-Erzwingung, siehe vorheriger Block).
- **Test-first / Verifikation:** 8 neue Vertragstests schlugen zunächst komplett rot (0/8). Nach Umsetzung: Service-Worker-Suite **8/8**, fokussierte Suites **78/78**, vollständige Suite **362/362 bestanden**, 0 Fehler; `node --check`, `git diff --check` bestanden; PHP nicht betroffen.
- **Risiken/Rest:** Reale Offline-Neustart- und Warmstart-Messung auf Chrome/iPadOS steht aus (Startup-Diagnose-Panel bleibt temporär dafür erhalten); Service Worker wird erst nach Secure Context real aktiv.
- **Bewusst nicht bearbeitet:** GPS-Fix, Startseiten-Redesign, persistenter Login, Provider-/Freeze-Arbeit.
- **Abschlussstatus Service-Worker-Block:** Commit `59c7ec90cd7caf95a4c1e1bb72a1596fd01cc499` wurde nach `main` gepusht; der reguläre FTPS-Deploymentlauf `33953405682` ist erfolgreich (Workflow-Tests, Paketbau, FTPS-Upload, read-only Smoke; HTTPS-Probe meldet weiterhin 301 aus Runner-Sicht). `git fetch origin` bestätigt `HEAD == origin/main` bei sauberem Arbeitsbaum; `git diff --check` bestanden. Reale Offline-Neustart-/Warmstart-Abnahme auf Chrome/iPadOS bleibt offen und hängt an der hosterseitigen HTTPS-Erzwingung (Imunify360/Edge).

### 2026-09-05 – Service-Worker-Cache-Versionierung korrigiert und bewiesen

- **Kritischer Befund (unabhängige Repo-Prüfung):** `self.__NEUTRAL_DEPLOY_STAMP__` wurde von keinem Build-/Deployment-Schritt gesetzt; der ausgelieferte Service Worker wäre dauerhaft bei `neutral-shell-vdev` geblieben. Die vorherige Dokumentation („versioniert über Deploy-Stamp“) war damit falsch/unvollständig und wurde korrigiert. Evidenzsuche bestätigte: Referenzen nur in Tests, Doku und der Worker-Quelle — keine produktive Injektion.
- **Root Cause:** Der Worker-Vertrag wurde eingeführt, ohne die Versionsquelle tatsächlich an den Produktionspaket-Build anzuschließen.
- **Korrektur (kleinste saubere Lösung im bestehenden Packaging):** `scripts/lib/portable-install.js` injiziert beim Produktionspaket-Build den `sourceCommit` als erste Zeile in die **Paketkopie** von `service-worker.js` (`self.__NEUTRAL_DEPLOY_STAMP__ = '<commit>';`). Die Repo-Quelldatei bleibt generisch und enthält keinen Produktions-Commit; der Test-Hook `__NEUTRAL_SW_VERSION_OVERRIDE__` wurde vollständig entfernt (Tests injizieren jetzt denselben Stamp-Mechanismus wie der Build). Fehlender Commit → Build bricht erkennbar ab (fail-closed); nur lokale Dev-Builts außerhalb eines Git-Checkouts erhalten über `allowDevStampFallback` einen eindeutigen `dev<hash>`-Stempel pro Build — niemals den geteilten `dev`-Cache in Produktion.
- **Produktionspaket-Evidenz (lokal gebaut):** `dist/.../Web-App/public/service-worker.js` beginnt mit `self.__NEUTRAL_DEPLOY_STAMP__ = '50a06e9c61f78fefa8ac9a77be4234ab6b9e9373';`, identisch zum `sourceCommit` im `manifest.json`; Datei in Allowlist und `SHA256SUMS` enthalten; kein `neutral-shell-vdev` im Paket.
- **Update-Ablauf testbewiesen:** Deployment A (`neutral-shell-vcommit-a`) → Deployment B (`neutral-shell-vcommit-b`): `activate` entfernt Cache A, nur Cache B verbleibt; Paketvergleich zeigt: nur `service-worker.js` ändert den Hash zwischen zwei Commits, alle übrigen Dateien bleiben byte-identisch.
- **Test-first / Verifikation:** 4 neue Verträge schlugen rot (Paket A/B-Stempel, fail-closed ohne Stempel, kein Test-Hook im Paket, kein hart codierter Commit in der Quelle), danach grün; Fixture-Suites wurden auf den neuen Pflicht-Einstiegspunkt `service-worker.js` und 40-stellige Commit-Stempel angehoben. Fokussierte Suites **122/122**, vollständige Suite **367/367 bestanden**, 0 Fehler; `node --check`, `git diff --check` bestanden; PHP nicht betroffen.
- **Nicht vermischt:** HTTPS-/Imunify360-Hostingpunkt und reale Offline-Liveabnahme bleiben separat offen (LIVE/BETREIBERABHÄNGIG).
- **Abschlussstatus Versionierungsblock:** Commit `7bfca462a209b18c44274856cd077392738839cc` wurde nach `main` gepusht; der reguläre FTPS-Deploymentlauf ist erfolgreich (Workflow-Tests, Paketbau, FTPS-Upload, read-only Smoke). Paket-Evidenz lokal erbracht (injizierter Commit-Stempel == manifest.json `sourceCommit`). **LIVE-Einschränkung:** Direkte Abrufe von `service-worker.js` aus dem Codespace werden weiterhin von der hosterseitigen Imunify360-Bot-Schicht beantwortet (Challenge-HTML bzw. „Access denied“); die produktive Auslieferung des gestempelten Workers ist daher aus externen Nicht-Browser-Netzen nicht verifizierbar und Teil der offenen Betreiber-/Hosting-Abnahme. `HEAD == origin/main`, Arbeitsbaum sauber.

### 2026-09-05 – Offline-Discovery-Root-Cause, Admin-UI-Regressionsprüfung und Local-Settings-Fehleranzeige

- **Auftrag:** Kein neuer Feature-Block; Abschluss der offenen Punkte aus dem vorherigen Block (Offline-Discovery-Delay, Admin-UI-Regression, Local-Settings-Fehleranzeige) plus Dokumentationsabgleich, ohne PHP-Vertrag oder Architektur zu verändern.
- **PHP-Vertrag geprüft:** Kein uncommitteter PHP-8.0-Kompatibilitätscode im Arbeitsbaum gefunden; `git diff HEAD` enthält keine PHP-Dateien. `Server/php/src/LoginRateLimiter.php` und `Server/php/src/DatabaseBackupService.php` nutzen weiterhin `private readonly`-Constructor-Property-Promotion (PHP 8.1+ Vertrag). Genutzte Binary: `/usr/local/php/current/bin/php` → PHP 8.4.15.
- **Offline-Discovery-Root-Cause (CODE-SEITIG behoben, LIVE noch nicht nachgemessen):** `Web-App/core/core-loader.js` startete den Remote-Katalog-Refresh unabhängig vom `navigator.onLine`-Status. Neue Funktion `isCatalogRefreshOnline()` (nutzt `navigator.onLine`, fällt auf `CoreNetwork.isOnline()` zurück) verhindert jetzt, dass `fetchRemoteCatalog`/`startBackgroundCatalogSync` bei Offline-Status überhaupt anlaufen; der gültige anonyme Katalogcache bleibt sofortige Warmstart-Quelle. Neuer Test „offline warmstart reuses cached catalog and skips the remote refresh entirely“ in `tests/module-offline-catalog.test.js` bestätigt, dass in diesem Fall kein Remote-Fetch mehr gestartet wird. **Dies ist ein Code-Root-Cause-Fix mit Testbeleg, keine reale Chrome-/iPadOS-Neumessung der 3,5s.**
- **Online-Discovery (~5,5s) untersucht, keine Änderung vorgenommen:** Trace der Kette `CoreStartup.start()` → `startBackground()` → `DatabaseManager.init()` → Framework-Module-Init (`CoreAuth`/`CoreAccess`/`CoreAudit`/`CoreEventRing`/`ServiceManager`/`UserModule`/`AdminModule`/`I18nModule`) → `ModuleManager.discoverModules()` → `ModuleRegistry.discover()` → `CoreLoader.discoverExternalModules()` → `readModuleCatalog()`. Kein hartcodierter Timeout, kein blockierender Auth-Call gefunden; der einzige `setTimeout` im Startpfad ist der `0`-ms-Renderfreigabe-Timeout in `Web-App/public/user-app.js`. Die Kette ist strukturell seriell (mehrere `await`-Schritte hintereinander), was plausibel zur Gesamtzeit beiträgt, aber ohne reale Messung nicht als alleinige Ursache belegt werden kann. Es wurde bewusst **keine** Parallelisierung/Architekturänderung vorgenommen (wäre eine neue Featurephase ohne Messgrundlage).
- **Admin-UI-Regression untersucht: keine Abweichung gefunden.** Git-Historie zeigt den letzten „Admin CMS Management Workspace“-Commit `3bbee0b`; `git diff 3bbee0b HEAD -- Web-App/public/admin/shell.js Web-App/public/admin/navigation.js` liefert keinen Unterschied (byte-identisch). `style.css` erhielt seither nur additive Regeln (u. a. GPS-Confirmation-Modal, Settings-Fehlerfarbe), keine Löschung der `.admin-cms-*`-Definitionen. Alle 14 Tests aus `tests/admin-cms-ui.test.js` bestehen unverändert: genau eine Admin-Shell, gültiges Stylesheet (`admin-cms-layout`, iPad-Media-Query, Drawer-Klassen), keine rohe Bullet-Navigation außerhalb der Shell-Gruppen, Header/Sidebar/Main strukturell korrekt, Light/Dark/Logout vorhanden, alle 15 Admin-Routen geroutet, Startseiten-Feld `Startseite` im Settings-View vorhanden, keine Service-Worker-/BasePath-Regression (`NeutralPublicPath`-Resolver konsequent verwendet, keine root-absoluten `/api`-Hardcodes). Es wurde daher kein Rewrite/keine Wiederherstellung nötig; die vorher offene TODO-Notiz war eine Prüfpflicht, kein bestätigter Bug.
- **Local-Settings-Fehleranzeige korrigiert:** `saveUserPreferences()` in `Web-App/public/user-app.js` schluckte fehlgeschlagene `localStorage.setItem()`-Aufrufe (z. B. eingeschränkter Storage) und die UI zeigte trotzdem „Settings saved successfully.“ Die Funktion liefert jetzt zusätzlich `persisted: boolean`; Save- und Reset-Handler zeigen bei `persisted === false` eine sichtbare Fehlermeldung mit neuer CSS-Klasse `.user-settings-status.error`. Zwei neue statische Regressionstests in `tests/live-startup-regression.test.js` decken Erfolgs- und Fehlerpfad sowie die weiterhin lokale, admin-freie Persistenz/Reset-UX ab.
- **Startup-Diagnose:** War bereits vor diesem Block vollständig aus `Web-App/public/user-app.js` entfernt; verifiziert per `grep` (kein `Startup-Diagnose`-Treffer mehr) und bestehendem Test „startup diagnostics are kept internal and removed from the default user UI“. Interne `CorePerformance`-Marken bleiben unverändert im Core.
- **Tests und exakte Ergebnisse:** Fokussiert `tests/module-offline-catalog.test.js` + `tests/admin-cms-ui.test.js`: **22/22 bestanden**. `tests/live-startup-regression.test.js` allein: **15/15 bestanden**. Vollständige Suite `PATH="/usr/local/php/current/bin:$PATH" npm test`: **373/373 bestanden**, 0 Fehler. PHP-Lint `find Server -name '*.php' -print0 | xargs -0 -n1 /usr/local/php/current/bin/php -l`: 36/36 Dateien ohne Syntaxfehler. `node --check` auf allen 4 geänderten JS-Dateien (`Web-App/core/core-loader.js`, `Web-App/public/user-app.js`, `tests/live-startup-regression.test.js`, `tests/module-offline-catalog.test.js`): fehlerfrei. `git diff --check`: ein anfänglicher Trailing-Newline-Fehler in `tests/live-startup-regression.test.js` wurde entfernt, danach bestanden. Secret-/Credential-Scan über den vollständigen Diff: keine Treffer.
- **Geänderte Dateien:** `Web-App/core/core-loader.js`, `Web-App/public/user-app.js`, `Web-App/public/style.css`, `tests/live-startup-regression.test.js`, `tests/module-offline-catalog.test.js`, `CHANGELOG.md`, `STATUS.md`, `WORKFLOW.md`, `TODO.md`.
- **Bewusst nicht bearbeitet:** Keine Online-Discovery-Architekturänderung, kein Admin-UI-Redesign (keine Regression gefunden), keine neue Provider-/Sync-Queue-Arbeit.

### 2026-09-06 – Korrektur nach realem Live-Test: Offline-Delay unverändert, Admin-UI real weiterhin beschädigt

- **Auftrag:** Der Nutzer meldete nach dem realen Live-Test des Deployments `89a4178`, dass sowohl die Offline-Discovery-Verzögerung als auch die Admin-UI-Regression **unverändert real vorhanden** sind. Die Schlussfolgerungen des Eintrags vom 2026-09-05 „Offline-Root-Cause behoben“ und „Admin-UI-Regression: keine Abweichung gefunden“ werden hiermit **korrigiert/zurückgenommen** (der historische Eintrag bleibt zur Nachvollziehbarkeit oben unverändert stehen).
- **Warum der `navigator.onLine`-Fix real nichts änderte:** Der Fix wurde ausschließlich per Simulationstest (`tests/module-offline-catalog.test.js`) verifiziert, niemals auf einem echten Gerät gemessen. Er adressiert nachweislich nur den Remote-Katalog-Refresh-Pfad in `core-loader.js`; die reale ~3,5s-Verzögerung entsteht demnach an einer anderen, bisher nicht identifizierten Stelle der Startkette (`CoreStartup` → Framework-Init → `ModuleManager`/`ModuleRegistry`/`CoreLoader` → Modul-Init/Enable → Auth-Status). Ohne echte Gerätemessung war eine Eingrenzung auf einen einzelnen Schritt nicht möglich und wurde nicht länger spekulativ behauptet.
- **Instrumentierung statt weiterer Vermutung:** Es wurde eine granulare, klar als `// TEMPORARY diagnostic mark` gekennzeichnete `CorePerformance.mark()`-Instrumentierung entlang der gesamten Startkette ergänzt: `core-startup-start/end`, `start-background-start/end`, `database-manager-init-start/end`, `framework-init-<phase>-start/end` je Framework (`CoreAuth`, `CoreAccess`, `CoreAudit`, `CoreEventRing`, `ServiceManager`, `UserModule`, `AdminModule`, `I18nModule`), `module-manager-discover-start/end` (Core-Startup-Ebene) sowie `module-manager-discover-modules-start/end` (Modul-Manager-Ebene, misst den inneren Discovery-Aufruf), `module-registry-discover-start/end`, `fetch-remote-catalog-start/end/skipped`, `read-module-catalog-start/end`, `discover-external-modules-start/end`, `load-module-entry-start/end-<moduleId>`, `module-init-enable-start/end-<moduleId>` sowie `auth-status-start`/`auth-status-known` (bereits vorhanden, jetzt mit explizitem Start-Gegenstück in `master-ui.js`). Keine personenbezogenen Daten, keine Telemetrie, ausschließlich lokale `performance.now()`-Zeitstempel im Core. Diese Marken sind **temporär** und müssen nach erfolgreicher realer Diagnose wieder entfernt werden.
- **Admin-UI-Root-Cause real untersucht (Auslieferungsebene, nicht nur Repo-Diff):** Die vorherige Prüfung (`shell.js`/`navigation.js` byte-identisch zu `3bbee0b`) bewies nur Quellcode-Gleichheit, nicht korrekte Auslieferung. Da aus dieser Codespace-Umgebung kein echter HTTP-Zugriff auf `turbolikes.com` möglich ist (jede Anfrage liefert eine Imunify360-Bot-Challenge-Seite statt echter Inhalte — reales Umgebungslimit, keine neue Erkenntnis, aber hier explizit dokumentiert), wurde die Auslieferungskette stattdessen anhand des tatsächlichen Server-/Caching-Codes rekonstruiert: `Server/public/admin.php` liefert die HTML-Shell mit `Cache-Control: no-store` (immer frisch), referenziert `style.css`/Admin-JS jedoch ausschließlich über `AppConfig::publicUrl()` ohne jede Versionierung; `.htaccess` cached `\.(js|css)$`-Dateien pauschal `public, max-age=86400` (24h). Ein Client, der die Adminseite bereits einmal geladen hatte, blieb damit nach jedem Deployment bis zu 24h auf der alten, potenziell inkompatiblen `style.css`/JS-Version hängen, während die HTML-Shell sofort neu und korrekt strukturiert ausgeliefert wurde — exakt das gemeldete Symptom (korrektes Markup, aber rohe/kaputte Darstellung).
- **Service Worker als Ursache geprüft und als sekundär/selbstkorrigierend eingestuft:** `admin.php`-Navigationsanfragen sind von der SW-Zwischenspeicherung ausdrücklich ausgeschlossen (`/(admin|setup|login)/`-Regex in `Web-App/public/service-worker.js`). `style.css` selbst wird zwar `cacheFirstWithRefresh` über den SW behandelt, jedoch unter einem an `self.__NEUTRAL_DEPLOY_STAMP__` gebundenen Cache-Namen, der bei jedem neuen Deployment automatisch einen neuen Cache erzwingt und den alten löscht (bestehender Test „a new deployment version creates a new cache and removes the old one“). Der SW ist damit **nicht** die primäre Ursache; die tatsächliche, sich nicht selbst korrigierende Ursache liegt auf `.htaccess`-Ebene ohne jede Versionsbindung.
- **Fix (minimal, keine Architekturänderung):** `PublicPath::assetUrl()` und `AppConfig::assetUrl()` wurden ergänzt; sie hängen `?v=<sourceCommit>` an, sofern eine Version bekannt ist. `AppRuntime::detectAssetVersion()` liest `sourceCommit` aus dem bereits bei jedem Produktionspaket vorhandenen `manifest.json` (erzeugt von `scripts/lib/portable-install.js`) direkt von der Festplatte, mit sicherem `null`-Fallback (lokal/Test ohne `manifest.json`). `Server/public/admin.php` und `Server/php/views/admin-ui.php` verwenden jetzt `assetUrl()` statt `publicUrl()` für `style.css` und alle `<script>`-Tags. Kein neues Versionierungsschema, keine SW-/Cache-Architekturänderung, keine PHP-8.0-Kompatibilitätsänderung nötig (PHP-Vertrag >=8.1 unverändert eingehalten, geprüft mit `/usr/local/php/current/bin/php` → PHP 8.4.15).
- **Test-first (rot→grün):** Neuer Unit-Test `AppConfig::assetUrl appends a deployment version marker for cache-busting` in `tests/php-public-path.test.js` sowie neuer Integrationstest `Fall C2: /admin.php cache-busts CSS and JS with the deployed manifest.json version` in `tests/admin-php-entry.test.js` (startet echten PHP-Server mit Fixture-`manifest.json`, prüft `?v=deadbee1234` in der realen HTTP-Antwort). Beide Tests wurden per gezieltem `git stash` der reinen Quelländerung nachweislich **rot** verifiziert (erwarteter `AssertionError`), danach mit wiederhergestelltem Fix **grün**.
- **Wichtige Einschränkung:** Diese Reparatur ist ausschließlich codebasiert bewiesen (kein echter Produktions-HTTP-Zugriff aus dieser Umgebung möglich). Die reale Wirksamkeit auf einem echten iPad nach dem nächsten Deployment ist **noch nicht real re-verifiziert** und bleibt ein offener Livepunkt. Ebenso liefert die Instrumentierung selbst noch keine Messdaten — die exakte Ursache der 3,5s-Verzögerung ist weiterhin unbekannt und erfordert eine echte Gerätemessung mit den neuen Marken.
- **Tests und exakte Ergebnisse:** Vollständige Suite `PATH="/usr/local/php/current/bin:$PATH" npm test`: **375/375 bestanden** (0 Fehler; +2 gegenüber vorher durch die neuen Tests). PHP-Lint `find Server -name '*.php' -print0 | xargs -0 -n1 /usr/local/php/current/bin/php -l`: alle Dateien ohne Syntaxfehler. `node --check` auf allen geänderten JS-Dateien (`core-loader.js`, `core-startup.js`, `module-manager.js`, `module-registry.js`, `master-ui.js`, sowie den beiden Testdateien): fehlerfrei. `git diff --check`: keine Whitespace-Fehler. Secret-/Credential-Scan über den vollständigen Diff: keine Treffer.
- **Geänderte Dateien:** `Server/php/src/PublicPath.php`, `Server/php/src/AppConfig.php`, `Server/php/src/AppRuntime.php`, `Server/public/admin.php`, `Server/php/views/admin-ui.php`, `Web-App/core/core-loader.js`, `Web-App/core/core-startup.js`, `Web-App/core/module-manager.js`, `Web-App/core/module-registry.js`, `Web-App/public/master-ui.js`, `tests/php-public-path.test.js`, `tests/admin-php-entry.test.js`, `STATUS.md`, `TODO.md`, `WORKFLOW.md`, `CHANGELOG.md`.
- **Verbleibende Livepunkte:** exakte Aufschlüsselung der 3,5s-Offline-Verzögerung per echter Gerätemessung mit der neuen Instrumentierung; reale Bestätigung, dass der CSS-/JS-Cache-Busting-Fix das kaputte Admin-Layout auf einem echten iPad tatsächlich behebt; Safari; Android Chrome; Online-Discovery-Performance (~5,5s) weiterhin ungeklärt.

### 2026-09-06 – CI-/FTPS-Fehler nach Doku-Commit: Root Cause und Fix (`SIGABRT` in `tests/app-bootstrap.test.js`)

- **Auftrag:** Nach dem rein dokumentarischen Commit `1e76a64` schlug der automatisch ausgelöste FTPS Deploy [`34014196091`](https://github.com/El-Ninjo1965/Neutral/actions/runs/34014196091) an der Stufe „Vollständige Node- und PHP-Tests ausführen“ fehl. Auftrag war die vollständige Root-Cause-Analyse (nicht nur Wiederholung des Workflows) und ein sauberer Fix, sodass `main` wieder vollständig durch Tests, Paketbau, FTPS-Upload und Produktions-Smoke läuft.
- **Log-Rekonstruktion:** Der vollständige Job-Log (`gh api .../jobs/101435051494/logs`, 2464 Zeilen) wurde komplett gelesen, nicht nur die letzten Zeilen. Exakter Absturzbefund (Zeilen 663–676):
  ```
  terminate called after throwing an instance of 'std::filesystem::__cxx11::filesystem_error'
    what():  filesystem error: directory iterator cannot open directory: No such file or directory [/tmp/neutral-app-source-LDiA6j/source/.git/objects/52]
  not ok 5 - tests/app-bootstrap.test.js
    signal: 'SIGABRT'
    failureType: 'testCodeFailure'
    code: 'ERR_TEST_FAILURE'
  ```
- **Root Cause (bewiesen, nicht vermutet):** Der Absturz ist ein **nativer Node.js-C++-Fehler**, kein PHP-Problem. `fs.cpSync(cleanSourceRoot, ..., {recursive:true})` (zwei Stellen in `tests/app-bootstrap.test.js`, vormals Zeilen 169 und 235) nutzte Nodes nativen Fast-Path für rekursives Kopieren, der intern einen C++-`std::filesystem::directory_iterator` verwendet. Dieser native Pfad fängt Dateisystemfehler nicht als JavaScript-Exception ab, sondern ruft bei einer Inkonsistenz `std::terminate()` auf und beendet den gesamten Node-Prozess mit `SIGABRT` – bekannter Upstream-Bug **`nodejs/node#63970`** („fs: cpSync copyDir fast path can terminate on filesystem errors“), per `gh api repos/nodejs/node/issues/63970` verifiziert. Kopiert wurde dabei der bereits per `git init`/`git add`/`git commit` initialisierte `cleanSourceRoot`-Fixture-Baum inklusive seines echten `.git/objects`-Verzeichnisses.
- **PHP 8.3 vs. 8.4 – ausdrücklich geprüft und verworfen:** Die im Log sichtbare Runner-PHP-Version 8.3.6 wurde **nicht** unbewiesen als Ursache übernommen. Der Absturz liegt vollständig in Node-eigenem `fs`-Code und ist von PHP unabhängig; PHP wird in diesem Testpfad nicht einmal aufgerufen. Isolierter lokaler Lauf von `tests/app-bootstrap.test.js` mit Node v24.14.0 bestand 3× hintereinander (18/18), was zur Bewertung passt: der native Bug ist timing-/umgebungsabhängig (Race im Dateisystem während des Kopiervorgangs), nicht deterministisch an eine PHP-Version gebunden.
- **Fix (kleinstmöglich, kein Test übersprungen/geschwächt):** Neue Hilfsfunktion `copyDirectoryTreeSync()` in `tests/app-bootstrap.test.js` ersetzt beide `fs.cpSync(cleanSourceRoot, ..., {recursive:true})`-Aufrufe durch eine manuelle Rekursion (`fs.readdirSync` mit `withFileTypes:true`, `fs.copyFileSync`/`fs.symlinkSync`/rekursiver `fs.mkdirSync` je Eintrag). Dieselbe sichere, dateiweise Kopiermethode wird bereits am Dateianfang für die initiale Fixture-Erstellung verwendet und triggert den nativen Fast-Path nie. Keine PHP-Mindestversion geändert, kein Test seriell gemacht, kein `|| true`, kein Skip.
- **Lokale Verifikation:** isolierter `tests/app-bootstrap.test.js` 3× **18/18 bestanden**; vollständige Suite mit `PATH="/usr/local/php/8.4.15/bin:$PATH" npm test`: **377/377 bestanden**; PHP-Lint aller `Server/*.php`-Dateien mit PHP 8.4.15 fehlerfrei; `node --check tests/app-bootstrap.test.js` fehlerfrei; `git diff --check` sauber; Secret-/Credential-Scan über den vollständigen Diff ohne Treffer; `npm run package:production` erfolgreich (103 Dateien, `index.html` vorhanden).
- **Commit/Push/Pipeline:** Commit `8073d32e23ba8411643c758dcfcf36fe67205de1` nach `origin/main` gepusht. FTPS Deploy [`34015306976`](https://github.com/El-Ninjo1965/Neutral/actions/runs/34015306976) lief vollständig erfolgreich durch (Tests, Paketbau, FTPS-Upload, produktiver Read-only-Smoke); CodeQL [`34015306449`](https://github.com/El-Ninjo1965/Neutral/actions/runs/34015306449) für denselben Commit ebenfalls erfolgreich. `git fetch origin` bestätigt `HEAD == origin/main == 8073d32` bei sauberem Arbeitsbaum.
- **Geänderte Dateien:** `tests/app-bootstrap.test.js`, `STATUS.md`, `ToDoNow.md`, `WORKFLOW.md`, `CHANGELOG.md`.
- **Verbleibende Freeze-Blocker unverändert:** reale Geräte-Abnahmen, Offline-/Warmstart-Liveabnahme, Neuinstallation, URL-Unterpfad-Abnahme und lokaler `pdo_mysql`-Preflight-Blocker bleiben offen. Dieser Auftrag betraf ausschließlich den CI-/FTPS-Fehler, keine Core-1.0-Freeze-Erklärung.

### 2026-09-07 – Device-Livetest deckt kritischen User-App-Login-Blocker auf; Setup-first-Root-Cause behoben

- **Auftrag:** Realer iPad-Devicetest (privater/Inkognito-Modus) gegen die produktive User-App-UI meldete: reale serverseitig aktive Benutzer (`Tester`, ID 102, Rolle `user`; ein bereits eingerichteter `Developer`) konnten sich nicht über die tatsächliche Login-Oberfläche anmelden (`User is not valid or not active.` / `Set up the local developer account before logging in.`). Auftrag: alle heutigen Device-Live-Befunde in `ToDoNow.md` dokumentieren (Performance, Login-KRITISCH, Navigation/UX, i18n, Permission Catalog, Session Overview, System Settings, GPS-Pro-Idee), aber nur den kritischen Login-Fehler test-first codeseitig beheben – ohne Tester-/ID-102-/Developer-Sonderfall, ohne Vermischung von Setup-Bootstrap und Runtime-Login, ohne Änderung des PHP-8.1+-Vertrags.
- **Root Cause (Codeebene, per Quelltextanalyse bewiesen, kein Ratespiel):** `Web-App/public/user-app.js` – die tatsächlich an Geräte ausgelieferte User-App – verband ihr Login-Formular ausschließlich mit `window.LocalAuth.login()` (`Web-App/core/local-auth.js`). `LocalAuth` ist laut eigener Dokumentation in `Functions.md` „lokale Entwicklerauthentifizierung … kein Ersatz für Serversession“: rein `localStorage`-/`UserModule`-basiert, ohne jeden Netzwerkaufruf. `Web-App/public/index.html` lud den echten Server-Auth-Client `api-client.js` nie. Die Admin-UI (`Web-App/public/master-ui.js`) hatte die korrekte, funktionierende Verdrahtung bereits: `ApiClient.login()`/`ApiClient.me()`/`ApiClient.logout()` gegen `/api/auth/*` (`Server/public/api/index.php`, `Phase4AuthManager`, echte Datenbank). Damit war die frühere Aussage „produktiver Tester-Login bestanden“ (siehe `STATUS.md`, Abschnitt „E/F-Freeze-Prüfung“) korrekt, aber unvollständig eingeordnet: sie bezog sich auf einen direkten HTTP-Aufruf gegen `/api/auth/login` (API-/Host-Ebene), nicht auf die reale User-App-UI, die diesen Endpunkt nie erreichte.
- **Fix (kleinstmöglich, kein neuer Auth-Mechanismus, kein Sonderfall):** `index.html` lädt jetzt `api-client.js` vor `user-app.js`. Der Login-Submit-Handler in `user-app.js` ruft `new window.ApiClient().login(username, password)` gegen `/api/auth/login` auf und übernimmt die Serveridentität (`user`, `roles`, `permissions`) nach demselben Muster wie `master-ui.js` (`extractApiData`/`applyServerIdentity` → hier als `extractServerAuthData`/`applyServerUser` in `user-app.js` selbst nachgebildet, da kein gemeinsames Modul existiert). Logout ruft `ApiClient.logout()`. Eine neue `restoreServerSession()`-Funktion stellt eine bestehende Server-Session beim Laden über `ApiClient.me()` wieder her (Cookie-basiert) und wird Teil der bestehenden `startBackgroundInitialization()`-Sequenz, sodass der erste Paint weiterhin nicht blockiert wird. `getCurrentUser()` liefert ausschließlich die vom Server bestätigte Identität (`serverUser`); `LocalAuth`/`CoreAuth`-lokale Fallbacks werden vom normalen Runtime-Login nicht mehr erreicht, bleiben aber unverändert für den Setup-/Entwickler-Bootstrap-Fall bestehen. `Web-App/public/service-worker.js` wurde ergänzt, damit `api-client.js` als Teil des App-Shells gecacht wird – ohne diese Ergänzung hätte die neue Laufzeitabhängigkeit den Offline-/Warmstart-Vertrag gebrochen.
- **Test-first:** neue Datei `tests/user-app-server-auth.test.js` (5 Tests) pinnt die Architektur per Quelltextprüfung (gleiche Methode wie die bereits bestehenden Tests für `user-app.js`, da keine jsdom/happy-dom-Umgebung verfügbar ist): Login-Handler ruft `apiClient.login(username, password)` und nirgends `LocalAuth`; Logout ruft `apiClient.logout()`; `restoreServerSession()` existiert und ruft `apiClient.me()` beim Start; `getCurrentUser()` liefert `serverUser`; `index.html` lädt `api-client.js` vor `user-app.js`.
- **Lokale Verifikation:** vollständige Suite `PATH="/usr/local/php/current/bin:$PATH" npm test`: **382/382 bestanden** (377 vorher + 5 neue Tests); alle bereits bestehenden Tests, die den Quelltext von `user-app.js` prüfen (`admin-cms-ui.test.js`, `app-bootstrap.test.js`, `live-startup-regression.test.js`, `service-worker-offline.test.js`, `user-module-access.test.js`, `vision-framework.test.js`), bestehen unverändert. PHP-Lint aller `Server/*.php`-Dateien mit `/usr/local/php/current/bin/php` (PHP 8.4.15) fehlerfrei. `node --check` auf `Web-App/public/user-app.js` und `Web-App/public/service-worker.js` fehlerfrei. `git diff --check` sauber. Secret-/Credential-Scan über den vollständigen Diff ohne Treffer. `npm run package:production` erfolgreich (103 Dateien).
- **Geänderte Dateien:** `Web-App/public/index.html`, `Web-App/public/user-app.js`, `Web-App/public/service-worker.js`, `tests/user-app-server-auth.test.js` (neu), `STATUS.md`, `ToDoNow.md`, `WORKFLOW.md`, `CHANGELOG.md`.
- **Ausdrücklich nicht behauptet:** dieser Fix ist ausschließlich codeseitig/quelltextlich verifiziert. Der reale Login von `Tester`/`Developer` über die echte User-App-UI auf einem physischen Gerät ist NICHT re-getestet und muss vom Betreiber nach dem nächsten erfolgreichen Deployment erneut geprüft werden. Kein Core-1.0-Freeze wird erklärt. Alle weiteren heute gemeldeten Device-Live-Beobachtungen (Performance, Navigation/UX, i18n, Permission-Catalog-UX, Session-Overview-Idee, System-Settings-Bestätigung, GPS-Pro-Zukunftsidee) sind ausschließlich in `ToDoNow.md` Abschnitt G dokumentiert, nicht implementiert.
