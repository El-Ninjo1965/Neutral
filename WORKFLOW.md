...
The current verified state is: the dedicated web-app FTP account is modeled as a chrooted root `/` target containing the public bundle, including `index.html`, `style.css`, `user-app.js`, `api-client.js`, and `platform/`. The repository-side root cause was the deploy script itself: `--web-app` mode had not been honored, so the wrong allowlist was staged for upload. That script bug has been corrected in `scripts/manual-ftps-deploy.js` so the public web-app bundle is now staged for the dedicated web-app target and not the server path. The verified live runtime path remains `/index/web-app/` and resolves to `/home/web1819/public_html/index/web-app/`; however, the actual live upload remains blocked by the current FTPS credential in `.env.web-app.deploy`: the account still rejects authentication (`530 Login authentication failed`), which means the local password must be updated to the active cPanel credential before a successful live deploy can be completed.
