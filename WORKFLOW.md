...
- **Angepasste Pfade:** JavaScript-/PHP-Imports, Manifest-Discovery, Node-Adapter, Runtimepfade, HTML-Assets, Tests, package entry, Startskript, Preflight und manuelles Deployment verwenden ausschließlich `Web-App/` und `Server/`. Die GitHub-Workflowdatei konnte wegen fehlender Token-Berechtigung für Workflows nicht übertragen werden und bleibt als offener Punkt dokumentiert.
...
- **Offene Punkte:** GitHub-FTPS-Workflow nach Erteilung der Workflow-Schreibberechtigung migrieren; direkte UI-Fetches vollständig auf den zentralen Transportservice konsolidieren; Hintergrundphasen messbar machen; produktive Offline-Sync-/Konfliktengine, Geräte-Matrix und PHP-Login-Drosselung bleiben gemäß `TODO.md` offen.
