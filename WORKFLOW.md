...
* Das PHP-Setup wurde mit den fehlenden Actions ergänzt: `Save configuration`, `Test server`, `Test database`, `Activate system`, sowie einer einmaligen lokalen Developer-Account-Initialisierung (`Developer` / Passwort und Bestätigung), damit die Funktionalität der bisherigen `setup.html`-Version in der PHP-Seite konsistent verfügbar ist.