...
- **Realer Diagnosebefund (Chrome/iPadOS, Produktion turbolikes.com):** Secure Context **NEIN**, Geolocation API JA, Permissions API JA, Permission State `denied`, `getCurrentPosition` aufgerufen JA, Ausgang `error`, Fehler `PERMISSION_DENIED` nach ca. 1 ms, Neutral-Consent erforderlich NEIN / angezeigt NEIN, letzte Neutral-Entscheidung JA. Damit ist bewiesen: Der Browserrequest wird tatsächlich ausgelöst und vom Browser sofort abgelehnt; der frühere Block liegt nicht mehr im Modulcode.
- **Root Cause `PERMISSION_DENIED`:** Geolocation erfordert einen Secure Context. Da `window.isSecureContext === false` ist, lehnt der Browser die Positionsabfrage sofort mit Code 1 ab. Das sofortige `denied` der Permissions-API ist damit konsistent und keine separate gespeicherte Standortverweigerung im Modul.
...
- **Teil A – GPS/Transport (realer Beweis, Chrome auf iPadOS):** Das temporäre Diagnosepanel zeigt jetzt eindeutig: Secure Context NEIN, **Protokoll: http:**, Eingebettet (Frame) NEIN, `getCurrentPosition` aufgerufen JA, Ausgang error mit `PERMISSION_DENIED` nach ca. 1 ms. Damit ist bewiesen: Die reale App-Seite läuft auf dem getesteten Gerät über HTTP, der GPS-Code ruft die API korrekt auf, der Browser blockiert im unsicheren Kontext. Produktionsprobe aus dem Codespace bestätigt: `http://turbolikes.com/`, `/app/` und `http://www.turbolikes.com/` antworten alle mit **200 ohne Redirect**. Root Cause liegt im Auslieferungspfad: Die Root-`.htaccess` enthielt bisher **keine** HTTPS-Erzwingung (sie wird per FTPS ins Ziel deployed, aber es gab keine Regel).
