  ...
  editor(packageData = { modules: {} }) {
    ...
    host.querySelector('form').addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (result.ok) { window.NeutralUiFeedback?.showSuccess(packageData.id ? 'Package updated successfully.' : 'Package created successfully.'); await this.init(this.container); } else AdminCommon.showAlert(result.error, 'error');
  ...
  editor(license = {}) {
    ...
    host.querySelector('form').addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (result.ok) { window.NeutralUiFeedback?.showSuccess(license.id ? 'License updated successfully.' : 'License created successfully.'); await this.init(this.container); } else AdminCommon.showAlert(result.error, 'error');
      } finally { if (submit.isConnected) submit.disabled = false; }
    });
  }
}
