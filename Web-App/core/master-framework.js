(function (globalObject) {
  ...
  const FrameworkRuntime = {
    ...
    getBackupService() {
      if (typeof require === 'function') {
        try {
          return require('../../Server/node/services/backup-service');
        } catch (error) {
          return null;
        }
      }
    ...
    getRuntimeSetupDefaults() {
      ...
      const config = (() => {
        ...
        try {
          return require('../../Server/node/config') || {};
        } catch (error) {
          return {};
        }
      })();
    ...
    readPersistedSetupState() {
      ...
      if (typeof process !== 'undefined' && process.versions && process.versions.node) {
        try {
          ...
          const stateFile = path.resolve(process.cwd(), 'Server', 'node', 'runtime', 'setup-state.json');
    ...
    getDatabaseConfig(source = {}) {
      if (typeof require === 'function') {
        try {
          const databaseModule = require('../../Server/node/database/connection');
    ...
    saveSetupState(nextState = null) {
      ...
      if (typeof process !== 'undefined' && process.versions && process.versions.node) {
        try {
          const fs = require('node:fs');
          const path = require('node:path');
          const stateDir = path.resolve(process.cwd(), 'Server', 'node', 'runtime');
    ...
    getReleaseState() {
      if (typeof require === 'function') {
        try {
          const releaseService = require('../../Server/node/services/release-service');
    ...
    readPersistedAdminState() {
      ...
      if (typeof process !== 'undefined' && process.versions && process.versions.node) {
        try {
          const fs = require('node:fs');
          const path = require('node:path');
          const stateFile = path.resolve(process.cwd(), 'Server', 'node', 'runtime', ADMIN_STATE_FILE_NAME);
          if (fs.existsSync(stateFile)) {
            const raw = fs.readFileSync(stateFile, 'utf8');
            if (raw && raw.trim()) {
              candidates.push(JSON.parse(raw));
