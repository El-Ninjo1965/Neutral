...
- **2026-09-05 – Admin-UI-Regression geprüft, keine Abweichung gefunden:** Git-Historie mit letztem Admin-CMS-Commit `3bbee0b` verglichen; `Web-App/public/admin/shell.js` und `Web-App/public/admin/navigation.js` sind byte-identisch zu HEAD, `style.css` erhielt seither nur Ergänzungen. Alle 14 Admin-CMS-Pflichttests (`tests/admin-cms-ui.test.js`) bestehen weiterhin: eine Admin-Shell, gültiges Stylesheet, keine rohe Bullet-Navigation, Header/Sidebar/Main korrekt, Light/Dark/Logout, alle 15 Routen, Startseiten-Konfiguration, keine Service-Worker-/BasePath-Regression. Kein Code-Fix nötig; die reale iPad/Safari-Liveabnahme bleibt weiterhin offen und wird nicht als bestanden markiert,
...
- **2026-09-05 – CODE-ROOT-CAUSE behoben, LIVE noch nicht nachgemessen:** `Web-App/core/core-loader.js` startete den Remote-Katalog-Refresh bislang unabhängig vom Online-Status; ein neuer `navigator.onLine`/`CoreNetwork.isOnline()`-Guard unterbindet den Remote-Refresh jetzt vollständig im Offline-Zustand (Test: `tests/module-offline-catalog.test.js`, „offline warmstart reuses cached catalog and skips the remote refresh entirely“, 22/22 fokussiert bestanden). Dies ist ein Code-Fix mit Testbeleg für die ~3,5s-Offline-Verzögerung, **keine** reale Chrome-/iPadOS-Neumessung. Die Root-Cause für die separate ~5,5s-Online-Discovery-Zeit ist untersucht (serielle `await`-Kette in `CoreStartup`/`ModuleManager`/`ModuleRegistry`/`CoreLoader`, kein hartcodierter Timeout gefunden), aber ohne reale Messung nicht abschließend isoliert; keine Architekturänderung vorgenommen,
