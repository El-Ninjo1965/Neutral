...
- **DRINGEND – Live-Test 2026-09-05 (Chrome auf iPadOS) NICHT BESTANDEN / Secure Context beheben:** Die Diagnose belegt `Secure Context: NEIN` bei `PERMISSION_DENIED` nach ca. 1 ms. Ursache liegt außerhalb des Repositorys (Hosting-/Edge-Pfad: vorgeschaltete Challenge-Seite, `http://turbolikes.com/` ohne HTTPS-Redirect). Betreiberaufgaben: Seite nur über `https://` aufrufen, HTTP→HTTPS-Zwang im Hosting aktivieren, Anti-Bot-/Challenge-Funktion des Hosters prüfen. Danach GPS erneut live abnehmen; das **TEMPORÄRE** Panel `GPS-Diagnose (temporär)` zeigt zusätzlich Protokoll und Frame-Status und ist nach erfolgreicher Diagnose zu entfernen,
