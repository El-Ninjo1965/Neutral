...
- **DRINGEND – Live-Test 2026-09-05 (Chrome auf iPadOS) NICHT BESTANDEN / HTTPS-Zwang verifizieren:** Die Diagnose belegt `Protokoll: http:` mit `PERMISSION_DENIED` nach ca. 1 ms. Repo-Seite erledigt: Root-`.htaccess` erzwingt jetzt 301→HTTPS (Pfad/Query erhalten, keine Schleife) und der Produktions-Smoke fordert die Weiterleitung explizit. **Betreiberaufgabe (LIVE):** Falls HTTP weiterhin ohne Redirect antwortet, greift der vorgeschaltete Edge-/Hosting-Layer vor der `.htaccess` — dann HTTPS-Erzwingung im Hosting/Control-Panel/CDN aktivieren. Danach GPS erneut live abnehmen (Erwartung: Secure Context JA, Protokoll https:, Frame NEIN); das **TEMPORÄRE** Panel `GPS-Diagnose (temporär)` ist nach erfolgreicher Diagnose zu entfernen,
- **DRINGEND – Warmstart real messen (Chrome/iPadOS):** Nach diesem Deployment die **TEMPORÄRE** `Startup-Diagnose (temporär)` auf der Startseite ablesen (Marken in ms seit Navigationsstart) und melden; Flugmodus-Neustart nach erfolgreichem Online-Besuch prüfen. Anschließend über den dokumentierten VORSCHLAG (Service Worker / App-Shell-Caching, ggf. versioniertes Core-Bundle) entscheiden,
- **Offen – separat zu entscheiden (nicht begonnen):** Startseite soll neutral bleiben; später soll der Admin ggf. ein Modul bzw. einen Startinhalt als Startseite festlegen können,
