...
- **DRINGEND – Live-Test 2026-09-05 (nach Deployment `8ea5761`) NICHT BESTANDEN / erneut abnehmen:** GPS auf realem iPad/Safari weiterhin ohne Position (manuell, automatisch, Auto-Setting) und ohne sichtbares Consent-Modal. Zum nächsten Test steht in der GPS-Ansicht ein **TEMPORÄRER** Bereich `GPS-Diagnose (temporär)` bereit; die angezeigten Werte (Secure Context, Geolocation-/Permissions-API, Permission State, getCurrentPosition aufgerufen, letzter Ausgang, letzter Fehlercode, Consent-Felder, Zeitpunkt/Dauer) sind an den Entwickler zu melden. **Der Diagnosebereich ist nach erfolgreicher realer Fehlerdiagnose wieder zu entfernen.** Er enthält keine Koordinaten und sendet nichts an den Server,
