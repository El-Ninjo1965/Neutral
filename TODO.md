...
- **DRINGEND – nicht bestanden / erneut abnehmen:** GPS auf realem iPad/Safari erneut prüfen: automatische Positionsermittlung beim Öffnen (Auto-Setting EIN), manuelles `Position aktualisieren`, Consent-Modal (Auto AUS + Ja/Nein) und konkrete Fehlercodes; zur Diagnose in der Konsole `GpsModule.getDiagnostics()` auswerten (secureContext, geolocationAvailable, permissionsApiAvailable, permissionState, getCurrentPositionCalled, lastOutcome, lastErrorCode); technische Details nur in Diagnose/Test, nicht ungefiltert im User-UI,
