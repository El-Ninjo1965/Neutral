...
const readRuntimeConfig = (source = {}) => {
  ...
  if (base.type === 'sqlite') {
    base.path = pickString(config, ['path', 'storagePath'], pickString(env, ['DB_PATH', 'DATABASE_PATH'], 'server/runtime/data/framework.sqlite'));
  }
...
const createSqliteConnection = (config = {}) => {
  ...
  const storageDir = pathModule.resolve(process.cwd(), 'server', 'runtime', 'data');
  fs.mkdirSync(storageDir, { recursive: true });
  const databasePath = pathModule.resolve(process.cwd(), runtimeConfig.path || 'server/runtime/data/framework.sqlite');
