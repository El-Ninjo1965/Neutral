...
function ownsProcess(array $state,string $runner): bool {
    ...
    if (!is_string($cmdline)) return false;
    return str_contains($cmdline,$runner)&&str_contains($cmdline,$token);
}
...
if (($_SERVER['REQUEST_METHOD']??'GET')==='POST') {
    ...
    if (!hash_equals((string)$_SESSION['node_diagnostic_csrf'],$csrf)) {
        ...
        if (($manualState['status']??'')==='running'&&ownsProcess($manualState,$manualRunner)) {
            ...
            if (!isset($nodes[$selected])||!$nodes[$selected]['executable']) {
...
$manualStatus=[
 'status='.(string)($manualState['status']??'stopped'),
