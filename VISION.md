...
Der aktuelle Live-Check hat den tatsächlichen Stand bestätigt: Der Web-App-FTP-Account liefert das echte Web-App-Bundle im chrooted Root `/`; die öffentliche URL `/index/web-app/` läuft auf den dedizierten Web-App-Ordner und bleibt getrennt vom Neutral-Serverpfad `/index/app/neutral/webroot/`. Die eigentliche Ursache im Repository war der deploy-side Fehler: `--web-app` wurde nie richtig ausgewertet, sodass das Script immer den Server-Allowlist-Pfad statt des Web-App-Bundles vorbereitet hat. Dieser Fehler wurde im Deploy-Skript behoben. Die nachweisbare Live-Pfad-Ausrichtung bleibt dabei eindeutig auf den Web-App-Ordner unter `/index/web-app/` gerichtet; es besteht keine zusätzliche VHost- oder Alias-Annahme, die den korrekten Pfad erklärt.
