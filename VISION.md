...
Der aktuelle Live-Check hat den tatsächlichen Stand bestätigt: Der Web-App-FTP-Account ist als eigener, getrennter FTPS-Zielbereich für den Public-Client modelliert; die öffentliche URL `/index/web-app/` bleibt davon getrennt vom Neutral-Serverpfad `/index/app/neutral/webroot/`. Die eigentliche Ursache im Repository war der deploy-side Fehler: `--web-app` wurde nie richtig ausgewertet, sodass das Script immer den Server-Allowlist-Pfad statt des Web-App-Bundles vorbereitet hat. Dieser Fehler wurde im Deploy-Skript behoben. Der echte Live-Deploy ist jedoch weiterhin durch die aktuelle FTPS-Kennung des Web-App-Accounts begrenzt: Der Server lehnt den lokalen Login für `web-app@turbolikes.com` aktuell mit `530 Login authentication failed` ab, obwohl der öffentliche Pfad `/index/web-app/` korrekt auf `/home/web1819/public_html/index/web-app/` verweist. Solange diese lokale FTP-Kennung nicht mit dem aktuell gültigen cPanel-Kennwort übereinstimmt, kann der Web-App-Upload nicht erfolgreich abgeschlossen werden.
