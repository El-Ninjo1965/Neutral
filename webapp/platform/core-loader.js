...
(() => {
    ...
    const getCurrentAppRoot = () => {
        ...
        const webAppIndex = Math.max(segments.lastIndexOf('web-app'), segments.lastIndexOf('webapp'));
        if (webAppIndex > 0) {
            return `/${segments.slice(0, webAppIndex).join('/')}`;
        }
    ...
    const CoreLoader = {
        ...
        async discoverExternalModules(basePath = ((typeof require === 'function' && typeof process !== 'undefined') ? 'webapp/app/modules' : 'app/modules')) {
            const rootPath = (typeof basePath === 'string' && basePath.trim()) ? basePath.trim() : ((typeof require === 'function' && typeof process !== 'undefined') ? 'webapp/app/modules' : 'app/modules');
            const discovered = [];
