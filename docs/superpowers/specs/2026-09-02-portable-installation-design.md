...
`manifest.json` enthält Schema-Version, App-/Frameworkversion, Quellcommit wenn verfügbar, Erzeugungszeit, normalisierten Basispfad und die sortierte Dateiliste mit Größe und SHA-256. `SHA256SUMS` enthält dieselben Dateien in stabiler Sortierung. Manifest und Prüfsummendatei listen sich nicht selbst. Ein zweiter Lauf über unveränderte Eingaben muss dieselbe Nutzdateiliste und dieselben Nutzdatei-Hashes erzeugen; Zeitstempel und Quellcommit sind Metadaten und werden getrennt geprüft.
