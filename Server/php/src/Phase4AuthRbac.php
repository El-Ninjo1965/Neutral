...
final class Phase4SessionRegistry
{
    ...
    {
        try {
            $statement = $this->requireDatabase()->connect()->prepare("SELECT CASE WHEN EXISTS(SELECT 1 FROM license_users lux JOIN licenses lx ON lx.id=lux.license_id WHERE lux.user_id=:license_user AND lux.membership_status='active' AND lx.status='active') THEN (SELECT CASE WHEN lu.device_limit_mode='unlimited' THEN NULL WHEN lu.device_limit_mode='override' THEN lu.device_limit WHEN l.device_limit_mode='unlimited' THEN NULL WHEN l.device_limit_mode='override' THEN l.device_limit ELSE CASE WHEN JSON_EXTRACT(p.limits_json,'$.allowedDevices') IS NULL OR JSON_TYPE(JSON_EXTRACT(p.limits_json,'$.allowedDevices'))='NULL' THEN NULL ELSE CAST(JSON_UNQUOTE(JSON_EXTRACT(p.limits_json,'$.allowedDevices')) AS UNSIGNED) END END FROM license_users lu JOIN licenses l ON l.id=lu.license_id JOIN packages p ON p.id=l.package_id WHERE lu.user_id=:limit_user AND lu.membership_status='active' AND l.status='active' LIMIT 1) ELSE (SELECT CASE WHEN u.device_limit_mode='unlimited' THEN NULL WHEN u.device_limit_mode='override' THEN u.device_limit WHEN u.package_id IS NULL THEN :fallback_limit ELSE CASE WHEN JSON_EXTRACT(p.limits_json,'$.allowedDevices') IS NULL OR JSON_TYPE(JSON_EXTRACT(p.limits_json,'$.allowedDevices'))='NULL' THEN NULL ELSE CAST(JSON_UNQUOTE(JSON_EXTRACT(p.limits_json,'$.allowedDevices')) AS UNSIGNED) END END FROM users u LEFT JOIN packages p ON p.id=u.package_id AND p.status='active' WHERE u.id=:direct_user) END device_limit");
