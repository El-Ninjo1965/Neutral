...
const bootstrapDefaultApps = () => {
  ...
  const readAppManifests = () => {
    const appsRoot = path.join(rootDir, 'Web-App', 'apps');
...
const appModulesDir = path.join(rootDir, 'Web-App', 'app', 'modules');
...
const appendSetupLog = (message, context = {}) => {
  try {
    const logDir = path.join(rootDir, 'Server', 'node', 'runtime');
...
const sanitizeSetupStateForClient = (snapshot = {}) => {
  ...
  state.discovery = {
    ...
    project: {
      rootDir,
      webRootDir,
      appPath: path.join(rootDir, 'Web-App', 'app'),
      platformPath: path.join(rootDir, 'Web-App', 'core'),
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (manifest && manifest.id) {
          manifests.push({
            ...manifest,
            modulePath: `Web-App/app/modules/${entry.name}`
          });
        }
      } catch {
...
const routeApi = (url, res, modulesDir = appModulesDir, req = null) => {
  ...
  if (adminModuleActionMatch) {
    ...
    const normalizeModuleEntry = (entry) => ({
      ...
      modulePath: String(entry.modulePath || `Web-App/app/modules/${entry.id || entry.name || moduleId}`),
      installed: true
    });
  ...
  if (pathname === `${apiBase}/admin/modules` || pathname === `${apiBase}/admin/modules/`) {
    ...
    const modules = readAppModuleManifests(modulesDir).map((module) => ({
      ...
      modulePath: String(module.modulePath || `Web-App/app/modules/${module.id || module.name || 'unknown-module'}`),
      installed: true
    }));
...
const handleStaticRequest = (url, req, res, modulesDir) => {
  ...
  if (requestPath.startsWith('/Web-App/public/')) {
    requestPath = requestPath.replace(/^\/Web-App\/public\//, '/');
  }

  if (requestPath.startsWith('/core/')) {
    serveStaticFile(res, safeResolve(path.join(rootDir, 'Web-App'), requestPath));
    return;
  }

  if (requestPath.startsWith('/modules/')) {
    const modulePath = requestPath.slice('/modules/'.length);
