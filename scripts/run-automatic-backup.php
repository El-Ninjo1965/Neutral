...
$stateFile = $runtime->projectRoot() . '/Server/runtime/backups/.automatic-state.json';
$state = is_file($stateFile) ? json_decode((string) file_get_contents($stateFile), true) : [];
if (is_array($state) && (int) ($state['lastSuccess'] ?? 0) > time() - $seconds) exit(0);

try {
    $service = new DatabaseBackupService($runtime->database(), new SchemaMigrator($runtime->database()), $runtime->config(), $runtime->projectRoot());
    $backup = $service->create();
    $service->enforceRetention(max(1, min(100, (int) ($options['backupRetention'] ?? 14))));
    file_put_contents($stateFile, json_encode(['lastSuccess' => time(), 'backupId' => $backup['backupId'], 'lastError' => null], JSON_THROW_ON_ERROR), LOCK_EX);
