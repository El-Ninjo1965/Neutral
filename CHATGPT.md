...
# 2026-09-11 minimal-Core architecture follow-up

Implemented and deployed in `4a912f5`: the shared User Login eye helper is now part of offline precache; module contracts distinguish lifecycle from User/Admin/system presentation and carry non-enforced optional dependencies; `SYSTEM-MODULES.md` defines concrete feature ownership, graceful absence, the Profile compatibility bridge, and Field Notes freeze proof. Full local suite passed 507/507 and the package contains 113 files. CodeQL `34534249430` and FTPS `34534248723` completed successfully; the workflow's bounded read-only smoke verified the deployed revision and `migrationsReady:true`. Profile has not been falsely claimed as extracted, and Core Freeze remains blocked on that non-destructive migration and Field Notes proof. No production restore or destructive production action was performed. Operator retest: User Login eye/offline refresh, unchanged commercial flows, and later Profile-disabled/system-module behavior after extraction.
