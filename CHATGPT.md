...
Implemented locally: the shared User Login eye helper is now part of offline precache; module contracts distinguish lifecycle from User/Admin/system presentation and carry non-enforced optional dependencies; `SYSTEM-MODULES.md` defines concrete feature ownership, graceful absence, the Profile compatibility bridge, and Field Notes freeze proof. Profile has not yet been falsely claimed as extracted, and Core Freeze remains blocked on that non-destructive migration and proof. No production restore or destructive production action was performed. Deployment evidence follows after CI.
